...
describe SessionsController do
  ...
  describe "on successful login," do
    before do 
      @user  = mock_user
      @login_params = { :email => 'quentin@gmail.com', :password => 'test' }
      User.should_receive(:authenticate).with(@login_params[:email], @login_params[:password]).any_number_of_times.and_return(@user)
    end
        ...
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            before do
              ...
              controller.should_receive(:cookies).any_number_of_times.and_return(@ccookies)
            ...
            it 'redirects to the home page'  do do_create; response.should redirect_to('http://test.host/customers/choose')   end
  ...
  describe "on failed login" do
    ...
    it 'flashes an error'           do do_create; flash[:error].should =~ /Couldn't log you in as 'quentin@gmail.com'/ end
