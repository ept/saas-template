...
#
Then "$actor should be on subdomain (.*) at '(.*)'" do |_, subdomain, path| 
  start = "http://" + subdomain + ".example.com/" + path

  current_url[0, start.length].should == start
end
...
Then "$actor should see an? <$container> containing a $attributes" do |_, container, attributes|
...
def have_flash notice, *args
  have_tag("[id=flash#{notice.capitalize}]", *args)
end
